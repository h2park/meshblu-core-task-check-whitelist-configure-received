http = require 'http'
CheckWhitelistConfigureReceived = require '../'

describe 'CheckWhitelistConfigureReceived', ->
  beforeEach ->
    @whitelistManager =
      checkConfigureReceived: sinon.stub()

    @sut = new CheckWhitelistConfigureReceived
      whitelistManager: @whitelistManager

  describe '->do', ->
    describe 'when called with a valid job', ->
      beforeEach (done) ->
        @whitelistManager.checkConfigureReceived.yields null, true
        job =
          metadata:
            auth:
              uuid: 'green-blue'
    ...
    describe 'when called with a valid job without a from', ->
      ...
      it 'should call the whitelistmanager with the correct arguments', ->
        expect(@whitelistManager.checkConfigureReceived).to.have.been.calledWith emitter: 'green-blue', subscriber: 'bright-green'
    ...
    describe 'when called with a job that with a device that has an invalid whitelist', ->
      beforeEach (done) ->
        @whitelistManager.checkConfigureReceived.yields null, false
        job =
          metadata:
            auth:
              uuid: 'puke-green'
    ...
    describe 'when called and the checkConfigureReceived yields an error', ->
      beforeEach (done) ->
        @whitelistManager.checkConfigureReceived.yields new Error "black-n-black"
        job =
          metadata:
            auth:
              uuid: 'puke-green'
